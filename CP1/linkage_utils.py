...
def evaluate_submission(target_curves_path = './target_curves.npy', results_folder = './results'):
    ...
    for i in trange(6):
        if os.path.exists(os.path.join(results_folder,'%i.csv'%i)):
            print('Evaluating Curve %i Solution'%i)

            mechanisms = get_population_csv(os.path.join(results_folder,'%i.csv'%i))
            F = []
            for m in mechanisms:
...
def visualize_pareto_front(mechanisms,F,target_curve):
    ...
    for i in trange(X_p.shape[0]):
        ...
        axs[i,1].set_title(f"Chamfer Distance: {CD:.5f} | Material Use: {mat:.2f}")
